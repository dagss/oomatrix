from .common import *
from ..task import Task, Argument, Scheduler, DefaultExecutor
...
class MockTasks(object):
    ...
    def build(self, name):
        ...
        if arg_names is None:
            # leaf
            node = Argument(len(self.arguments), META)
            ...
            node = Task(compute, 1 * FLOP, arg_tasks, META, None)
        self.tasks[name] = node
        return node

    def make_scheduler(self):
        return Scheduler(self.root, DefaultExecutor(self.arguments))

def test_basic():
    tasks = MockTasks('a', {'a': ['b', 'c'],
                            'b': ['d', 'e'],
                            'd': ['e']})
    e = tasks.make_scheduler()
